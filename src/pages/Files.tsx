...
export default function Files() {
  ...
  return (
    <div className="pt-8 pb-20 min-h-screen bg-base relative overflow-hidden text-text-main font-sans">
      <SEO 
        title="Comprehensive Careers Handbook For Forensic Science - Amazon Kindle Edition"
        description="Book Is Developed Under Initiative Of Foren Clue And Has Been Designed To Serve As A Comprehensive Guide For Individual Who Are Interested In Pursuing Education And Carrier In Forensic Science."
        keywords="forensic careers handbook, modern forensic science books, study materials forensic, amazon kindle forensic guide"
        canonicalPath="/files"
        image={bookCoverUrl}
      />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Minimal Breadcrumb */}
        <div className="mt-4 mb-10 flex items-center gap-2 text-[10px] font-mono tracking-wider text-text-muted uppercase">
          <span>Home</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-16 items-start mb-16 pb-16 border-b border-black/5 dark:border-white/5">
          ...
          <div className="md:col-span-4 flex flex-col items-center">
            <motion.div 
              ...
              className="relative w-full max-w-[280px] aspect-[3/4] rounded-lg overflow-hidden bg-surface border border-black/15 dark:border-white/10 shadow-md hover:shadow-lg transition-all duration-300"
            >
              {/* Premium minimal book cover */}
              <img 
                src={bookCoverUrl} 
            ...
            <div className="mt-5 w-full max-w-[280px]">
              <button 
                onClick={handleShare}
                className="w-full flex items-center justify-center gap-1.5 py-2 px-3 bg-surface hover:bg-surface-hover hover:border-text-muted/20 border border-black/10 dark:border-white/5 rounded-md text-[11px] font-medium transition-all text-text-muted hover:text-text-main"
              >
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-6 p-5 bg-surface/50 border border-black/5 dark:border-white/5 rounded-xl mb-6">
                ...
                <div className="flex-1 min-w-[200px]">
                  <a 
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-warning text-crust hover:bg-warning/90 rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-sm"
                  >
                    <ShoppingBag size={14} />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {bookSpecs.map((spec, idx) => (
                  <div key={idx} className="border border-black/5 dark:border-white/5 bg-surface/30 px-3 py-2 rounded-lg">
                    <span className="block text-[9px] font-mono text-text-muted uppercase tracking-wider">{spec.label}</span>
                    <span className="text-xs font-semibold text-text-main">{spec.value}</span>
                  </div>
                ))}
              </div>
              ...
              {ratingSubmitted ? (
                ...
                <form onSubmit={handleAddReview} className="space-y-3">
                  ...
                  <div className="text-right">
                    <button 
                      type="submit" 
                      className="px-3 py-1.5 bg-warning text-crust hover:bg-warning/90 rounded text-[10px] font-bold uppercase tracking-wider"
                    >
                      Submit Review
                    </button>
                  </div>
